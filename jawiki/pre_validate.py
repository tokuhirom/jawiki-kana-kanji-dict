...
class PreValidator:
    ...
    def validate(self, title, kanji, yomi) -> bool:
        if len(kanji) == 1:
            self.logger.info("Kanji is too short. " + f"title={title}, kanji={kanji}, yomi={yomi}")
            return False

        for yomi_prefix in ["[[", "いま、", "あるいは", "もしくは", "のうち、", "▢"]:
            if yomi.startswith(yomi_prefix):
                self.logger.info(f"ignorable yomi prefix: {yomi_prefix}. " + f"title={title}, kanji={kanji}, yomi={yomi}")
                return False

        for kanji_prefix in ["』", "[["]:
            if kanji.startswith(kanji_prefix):
                self.logger.info(f"ignorable kanji prefix: {kanji_prefix}." + f"title={title}, kanji={kanji}, yomi={yomi}")
                return False
